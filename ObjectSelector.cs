...
public class ObjectSelector : MonoBehaviour
{
    ...
    public List<ReflectionProbe> roomProbes = new List<ReflectionProbe>();

    public void UpdateAllProbes()
    {
        foreach (ReflectionProbe probe in roomProbes)
        {
            if (probe != null)
            {
                // Forces the probe to refresh its cubemap
                probe.RenderProbe();
            }
        }
    }
    ...
    // Called from JS: Click an active slot to reset it
    public void RestoreSlotOriginal(string indexStr)
    {
        if (_currentSelected == null) return;
        int slot = int.Parse(indexStr);
        foreach (var r in _selectedRenderers)
        {
            if (r != null) r.SetPropertyBlock(null, slot);
        }
        UpdateAllProbes();
    }
    ...
    public void ApplySaaSFinish(string data)
    {
        ...
        int slot = int.Parse(parts[0]);
        string albedoUrl = parts[1];
        string normalUrl = parts[2];
        string roughnessUrl = parts[3];
        string hex = parts[4];
        float smooth = float.Parse(parts[5], CultureInfo.InvariantCulture);
        float metallic = float.Parse(parts[6], CultureInfo.InvariantCulture);
        float tiling = float.Parse(parts[7], CultureInfo.InvariantCulture);
        float normStr = float.Parse(parts[8], CultureInfo.InvariantCulture);
        ...
        if (ColorUtility.TryParseHtmlString(hex, out Color col))
        {
            foreach (var r in _selectedRenderers)
            {
                ...
                // 1. Flat Paint Mode (If albedo is empty, clear textures)
                if (string.IsNullOrEmpty(albedoUrl))
                {
        ...
        // 3. Texture Caching logic using the Albedo URL as the key
        if (!_appliedTextures.ContainsKey(trackingKey) || _appliedTextures[trackingKey] != albedoUrl)
        {
            _appliedTextures[trackingKey] = albedoUrl;
            StartCoroutine(LoadPBRStreaming(slot, albedoUrl, normalUrl, roughnessUrl));
        }
    }

    private IEnumerator LoadPBRStreaming(int slot, string aUrl, string nUrl, string rUrl)
    {
#if UNITY_EDITOR
        // Convert to absolute file URIs for Editor testing
        if (aUrl.StartsWith("StreamingAssets")) aUrl = new System.Uri(System.IO.Path.Combine(Application.dataPath, aUrl)).AbsoluteUri;
        if (!string.IsNullOrEmpty(nUrl) && nUrl.StartsWith("StreamingAssets")) nUrl = new System.Uri(System.IO.Path.Combine(Application.dataPath, nUrl)).AbsoluteUri;
        if (!string.IsNullOrEmpty(rUrl) && rUrl.StartsWith("StreamingAssets")) rUrl = new System.Uri(System.IO.Path.Combine(Application.dataPath, rUrl)).AbsoluteUri;
#endif

        // 1. DOWNLOAD ALBEDO
        using (UnityWebRequest aReq = UnityWebRequestTexture.GetTexture(aUrl))
        {
            yield return aReq.SendWebRequest();
            if (aReq.result == UnityWebRequest.Result.Success)
            {
                ...
                // 2. DOWNLOAD NORMAL (Forcing Linear Color Space via raw bytes)
                if (!string.IsNullOrEmpty(nUrl))
                {
                    using (UnityWebRequest nReq = UnityWebRequest.Get(nUrl))
                    {
                        yield return nReq.SendWebRequest();
                        if (nReq.result == UnityWebRequest.Result.Success)
                        {
                            byte[] nData = nReq.downloadHandler.data;
                            normalMap = new Texture2D(2, 2, TextureFormat.RGBA32, true, true);
                            normalMap.LoadImage(nData);
                        }
                    }
                }

                // 3. DOWNLOAD ROUGHNESS (Optional)
                if (!string.IsNullOrEmpty(rUrl))
                {
                    using (UnityWebRequest rReq = UnityWebRequestTexture.GetTexture(rUrl))
                    {
                        yield return rReq.SendWebRequest();
                        if (rReq.result == UnityWebRequest.Result.Success)
                        {
                            roughnessMap = DownloadHandlerTexture.GetContent(rReq);
                        }
                    }
                }

                // 4. APPLY TO PROPERTY BLOCKS
                foreach (var r in _selectedRenderers)
                {
                    if (slot >= r.sharedMaterials.Length) continue;
                    MaterialPropertyBlock block = new MaterialPropertyBlock();
                    ...
                    block.SetTexture("_BaseMap", albedo);
                    if (normalMap != null) block.SetTexture("_BumpMap", normalMap);
                    
                    // URP Roughness mapping support
                    if (roughnessMap != null) 
                        block.SetTexture("_RoughnessMap", roughnessMap); 
            ...
            {
                Debug.LogWarning($"[ObjectSelector] Albedo download failed: {aReq.error} | URL: {aUrl}");
            }
        }
